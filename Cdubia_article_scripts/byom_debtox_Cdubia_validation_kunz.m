...
[data1,w1,LabelTable1] = data_AZT_Cdubia_kunz(TR,opt,1);
...
par.L0   = [0.368   0 0.2  0.4 1]; % body length at start experiment (mm)
par.Lp   = [0.6646   0 0.4    1  1]; % body length at puberty (mm)
par.Lm   = [0.8842     0 0.5    2  1]; % maximum body length (mm)
par.rB   = [0.2908   0 0.1    1  1]; % von Bertalanffy growth rate constant (1/d)
...
% Fit hb in data set
fit_tox = [-1 1 1]; % use both controls
...
fit_tox = [0 1 1]; % use both controls
par_out = automatic_runs_debtox2019(fit_tox,par,ind_tox,[],MOA,FEEDB,opt_optim,opt_plot);
% par_out = automatic_runs_debtox2019(fit_tox,par,ind_tox,[],MOA,FEEDB,opt_optim,opt_plot,opt_prof);
% script to run the calculations and plot, automatically
par = copy_par(par,par_out,1); % copy fitted parameters into par, and keep fit mark in par
