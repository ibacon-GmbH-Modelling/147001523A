...
% survivors
% S = load(fullfile('data_AZT_ceriodaphnia','Data_surv_Cdubia_AZT_calibration.txt'));
% S(1,1) = -1; % make sure to use multinomial likelihood for survival
% 
% % body length (mm) on each observation time (d)
% L = load(fullfile('data_AZT_ceriodaphnia','Data_length_Cdubia_AZT_calibration.txt'));
...
R = load(fullfile('data_AZT_ceriodaphnia','datakunz.txt'));
...
Cw = load(fullfile('data_AZT_ceriodaphnia','kunzexposure.txt'));
...
[~,loc_ctrl] = ismember([0],Cw(1,2:end));
Cw(:,loc_ctrl+1) = [];
...
Label = {'control';'6.25';'12.5';'25';'50';'100'};
...
DATA{1,2} = [0];   % length data
W{1,2}    = [0];  % length weights data
DATA{1,3} = R;   % reproduction data
W{1,3}    = RW;  % reproduction weights data
DATA{1,4} = [0];   % survival data
