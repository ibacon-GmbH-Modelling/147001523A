function [DATA,W,LabelTable] = data_AZT_Cdubia_validation2(TR,opt,study)

% Handy function to load the data sets from standard text files, and
% prepare them for BYOM analyses.
%
% This file: validation data set for AZT in Daphnia magna. 
% Specifications:
...
% survivors
S = load(fullfile('data_AZT','Data_surv_Cdubia_AZT_validation2.txt'));
S(1,1) = -1; % make sure to use multinomial likelihood for survival

% body length (mm) on each observation time (d)
L = load(fullfile('data_AZT','Data_length_Cdubia_AZT_validation2.txt'));
...
R = load(fullfile('data_AZT','Data_repro_Cdubia_AZT_validation2.txt'));
...
Cw = load(fullfile('data_AZT','Data_exposure_Cdubia_AZT_validation1.txt'));
Cw_type = 2; % block pulses   
...
Label = {'control';'solvent control';'close 1';'close 2';'close 3'};%;'wide 1';'wide 2';'wide 3'};
