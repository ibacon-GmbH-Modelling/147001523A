function [DATA,W,LabelTable] = data_AZT_Cdubia_validation1(TR,opt,study)
...
% survivors
S = load(fullfile('data_AZT','Data_surv_Cdubia_AZT_validation1.txt'));
S(1,1) = -1; % make sure to use multinomial likelihood for survival

% body length (mm) on each observation time (d)
L = load(fullfile('data_AZT','Data_length_Cdubia_AZT_validation1.txt'));
...
R = load(fullfile('data_AZT','Data_repro_Cdubia_AZT_validation1.txt'));
