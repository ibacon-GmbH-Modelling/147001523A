function [DATA,W,LabelTable] = data_AZT_Cdubia_calibration(TR,opt,study)

% Handy function to load the data sets from standard text files, and
% prepare them for BYOM analyses.
%
% This file: calibration data set for AZT in Ceriodaphnia Dubia.
% Specifications:
...
% survivors
S = load('data_AZT_ceriodaphnia\Data_surv_Cdubia_AZT_calibration.txt');
S(1,1) = -1; % make sure to use multinomial likelihood for survival

% body length (mm) on each observation time (d)
L = load('data_AZT_ceriodaphnia\Data_length_Cdubia_AZT_calibration.txt');
...
R = load('data_AZT_ceriodaphnia\Data_repro_Cdubia_AZT_calibration.txt');
...
Cw = load('data_AZT_ceriodaphnia\Data_exposure_Cdubia_AZT_calibration.txt');
...
Label = {'control';'solvent control';'early 100';'early 200';'early 400';'late 100';'late 200';'late 400'};
