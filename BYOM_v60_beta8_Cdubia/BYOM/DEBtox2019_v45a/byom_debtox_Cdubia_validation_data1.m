...
[data1,w1,LabelTable1] = data_AZT_Cdubia_validation1(TR,opt,1);
...
par.Lp   = [0.5   1 0.4    1  1]; % body length at puberty (mm)
par.Lm   = [1     1 0.5    2  1]; % maximum body length (mm)
par.rB   = [0.5   1 0.1    1  1]; % von Bertalanffy growth rate constant (1/d)
par.Rm   = [10    1   3   20  1]; % maximum reproduction rate (#/d)
par.f    = [1     0   0    2  1]; % scaled functional response
par.hb   = [0.01  1 1e-3 0.07 1]; % background hazard rate (d-1)
...
par.zs   = [0.1   1 0    1e6 1]; % effect threshold survival ([C])
par.bs   = [1     1 1e-6 1e6 0]; % effect strength survival (1/([C] d))
glo.Tbp=0; % needed because the parameter is not recognised
...
if scrsz(4) < 870 % on small screens ...
   opt_tktd.symsz    = 4; % size for the plotting symbol (default 6)
end
